...
> ⚠ **归因修正（2026-09-25 稍晚）**：后来在**回滚后的 CPU 路径**上抓开机日志也出现过 0 条 DSI 报错，
> 所以"报错归零"**不能**单独作为"PPA 在执行"的证据（它受画面变化量与时序影响，只能当参考）。
> PPA 确实跑起来的硬证据是**耗时特征的剧变**：每 block 从 0.6~0.9ms 变成 ~25ms ——
> 只有换了代码路径才可能造成这种量级变化。
