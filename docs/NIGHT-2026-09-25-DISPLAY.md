...
## 八、阶段版本固化（用户确认为「很不错的阶段版本」）

### 这一版包含什么
- **AXI-ICM QoS 提权**（0/0 → 15/15）：官方正解，开机 0 欠载。
- **E3 整块顺序读进 SRAM 再转置**：恶魔城 30 → 33 帧，峰值见过 39。
- **屏幕实时帧率数字**（每秒直写帧缓冲 + 先擦后画，位置在 L/R 肩键之间）。
- **DMA2D 保持开启**（关掉会撕裂：它的信号量就是唯一的 vsync 同步点）。
- **PPA 硬禁用**（见第七节结案）。
- **IDF 帧缓冲 128 对齐补丁**：`tools/patch-idf-dpi-fb-align.py`（幂等，拒绝猜 IDF 路径）。

### 真机实测（本阶段）
| 项目 | 数值 |
|---|---|
| 启动 | 0 欠载 / 0 面板报错 / BUSY 6% / FPS:104 |
| 恶魔城（E3 前 → 后） | 30 → 33，峰值 39 |
| 另一款游戏（稳定） | 屏显 **稳定 30**，BUSY 62~85% |
| 显示通路占用 | 306~425 ms/秒（每秒的 30~42%） |

### 关键发现：那"稳定 30"是什么
日志 `FPS: 60 (29+7+23)` 说明 —— **模拟器本身已经在跑 60fps**（60 次尝试），
**29 次被显示通路丢弃**，真正显示出去的 = 7+23 = **30**。
机制：每次推送要花约**两个面板刷新周期**（`draw=` 段 = DMA2D 拷贝 + 等它完成，
那个等待正好是一个完整 vsync）→ 流水线被锁在半速。
**结论：30 不是上限、不是模拟器不行，而是显示通路只跟得上 60 的一半。**
这是台阶而非线性 —— 把每帧显示开销压到 16.7ms 以内，有机会**直接跳到 ~58**。

### 归档
- 阶段快照：`dist/retro-go-tab5-stage-e3-fps.img`（2,424,832 字节，已验证可刷回）
- 下一步（已定）：**先 E2 双缓冲 → 再直写帧缓冲**（去掉 DMA2D 那趟拷贝与 vsync 等待）。
  上次直写在单缓冲地基上做导致蓝屏，顺序反了；这次多了一件工具：精确范围的 cache 写回
  （修帧率数字时验证过，只写回 115200 字节、128 对齐，不碰整帧 1.84MB）。
