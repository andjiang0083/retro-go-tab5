...
/* 虚拟按键可视层：把命中区画到屏幕上（0 = 不画，回到"盲区"）。
 * 实现在 mipi_dsi_tab5.h 的 lcd_send_buffer 里（推给面板前合成，才不会被 GUI 立即模式的
 * 重绘覆盖 => 不闪）；颜色在 tab5_overlay_color()，每个按键一个可区分的颜色。 */
...
#define RG_GAMEPAD_TOUCH_MAP { \
     ...
     * ⚠ 旧版是 2x2 紧贴网格（间距 92 = 尺寸 92 => 相邻方块正好贴死），实机反馈"太挤"。 \
     *   现在半径 90 / 尺寸 76 => 对角邻居轴对齐间隙 14px（无重叠），正交方向 104px。 \
