...
esp_err_t bsp_display_new_with_handles_to_st7123(const bsp_display_config_t* config, bsp_lcd_handles_t* ret_handles)
{
    ...
    ESP_LOGI(TAG, "LOCALDBG[1] enter: detect=%d is_st7121=%d", (int)display_type, (int)is_st7121); /* LOCAL PATCH: debug */

    ESP_RETURN_ON_ERROR(bsp_display_brightness_init(), TAG, "Brightness init failed");
    ESP_LOGI(TAG, "LOCALDBG[2] brightness ok"); /* LOCAL PATCH: debug */
    ESP_RETURN_ON_ERROR(bsp_enable_dsi_phy_power(), TAG, "DSI PHY power failed");
    ESP_LOGI(TAG, "LOCALDBG[3] dsi phy power ok"); /* LOCAL PATCH: debug */
    ...
    ESP_LOGI(TAG, "LOCALDBG[4a] creating dsi bus..."); /* LOCAL PATCH: debug */
    ESP_RETURN_ON_ERROR(esp_lcd_new_dsi_bus(&bus_config, &mipi_dsi_bus), TAG, "New DSI bus init failed");
    ESP_LOGI(TAG, "LOCALDBG[4b] dsi bus ok"); /* LOCAL PATCH: debug */
    ...
    ESP_GOTO_ON_ERROR(esp_lcd_new_panel_io_dbi(mipi_dsi_bus, &dbi_config, &io), err, TAG, "New panel IO failed");
    ESP_LOGI(TAG, "LOCALDBG[5] panel io ok"); /* LOCAL PATCH: debug */
    ...
    esp_lcd_dpi_panel_config_t dpi_config = {
        ...
        .video_timing =
            {
                .h_size            = BSP_LCD_H_RES,   /* LOCAL PATCH: 横向 */
                .v_size            = BSP_LCD_V_RES,   /* LOCAL PATCH: 横向 */
                .hsync_pulse_width = 2,
                .hsync_back_porch  = 40,
                .hsync_front_porch = 40,
        ...
        .flags =
            {
                /* use_dma2d 必须保持 true —— 2026-09-25 实验①的结论，别再关掉。
                 * 曾改成 false（CPU 同步拷贝），动机是：IDF 在有 DMA2D 时走「异步拷贝 +
                 * 0 超时抢信号量」，抢不到就丢弃本次绘制（日志 previous draw operation is
                 * not finished，实测每秒上百次）；且那条路每块要动约 120KB 内存
                 * （源写回 40K + 拷贝 40K + 目标写回 40K），真机打点显示 draw= 占每块 62% 时间。
                 * 关掉的实测结果：面板报错归零、不再丢弃 —— 但**出现横向割裂撕裂**，
                 * 因为「等在刷新完成中断里释放的那个信号量」本身就是我们唯一的 vsync 同步：
                 * 没有它，CPU 会直接往面板正在扫描的帧缓冲里写。
                 * 代价反而更大：拷贝从硬件搬到 CPU，BUSY 41%→64~79%，抢走模拟器算力，
                 * draw= 更不稳定（165~282ms vs 基线 270ms）。
                 * 结论：撕裂属用户可见属性，不可接受；要走这条路得先另找 vsync 同步手段。 */
                .use_dma2d = true,
            },
    };
    ...
    ESP_LOGI(TAG, "LOCALDBG[6a] creating st7123 panel driver..."); /* LOCAL PATCH: debug */
    ESP_GOTO_ON_ERROR(esp_lcd_panel_reset(disp_panel), err, TAG, "LCD panel reset failed");
    ESP_LOGI(TAG, "LOCALDBG[6b] panel reset ok"); /* LOCAL PATCH: debug */
    ESP_GOTO_ON_ERROR(esp_lcd_panel_init(disp_panel), err, TAG, "LCD panel init failed");
    ESP_LOGI(TAG, "LOCALDBG[6c] panel init ok"); /* LOCAL PATCH: debug */
    ESP_GOTO_ON_ERROR(esp_lcd_panel_disp_on_off(disp_panel, true), err, TAG, "LCD panel display on failed");
    ESP_LOGI(TAG, "LOCALDBG[7] panel display on ok"); /* LOCAL PATCH: debug */
    ...
    ESP_LOGI(TAG, "ST7123 Display initialized with resolution %dx%d", BSP_LCD_H_RES, BSP_LCD_V_RES);
...
esp_err_t bsp_touch_new(const bsp_touch_config_t* config, esp_lcd_touch_handle_t* ret_touch)
{
    ...
    const esp_lcd_touch_config_t tp_cfg = {
        .x_max        = 720,   /* LOCAL PATCH: 触摸 IC 自身是竖屏原生 720x1280，勿改 */
        .y_max        = 1280,  /* LOCAL PATCH: 同上 */
